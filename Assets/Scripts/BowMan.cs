...
public class BowMan : MonoBehaviour
{
    ...
    public void Attack()
    {
        if (Physics2D.Raycast(rayPoint.position, rayPoint.transform.right * direction, 5, whatIsEnemy))
        {
            isAttack = true;
        }
        else
        ...
        if (isAttack && isAllive)
        {
            ...
            if (Time.time >= timeToAttack)
            {
                ...
                Instantiate(arrow, rayPoint.position, rayPoint.rotation);
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireSphere(rayPoint.position, attackRange);
    }
}
